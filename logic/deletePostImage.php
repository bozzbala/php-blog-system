...
if(!isset($_SESSION['email'])) {
    ...
    if (isset($_GET['id']) && isset($_GET['image'])) {
        ...
        $user_id = $row['id'];
        $row2 = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM posts WHERE id='$id'"));

    if($user_id == $row2['user_id']){
        $query = "SELECT image_url FROM posts WHERE id='$id'";
        ...
        mysqli_query($conn, "UPDATE posts SET image_url = '$image_str' WHERE id='$id'");

        mysqli_close($conn);
        // redirect back to the product list page
        header("Location: /edit.php?id=" . $id . "&email=" . $email);
    }
    }